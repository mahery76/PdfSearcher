...
def search_pdf(pdf_path, search_term):
    ...
    try:
        ...
        with open(pdf_path, 'rb') as file, contextlib.redirect_stderr(io.StringIO()):
            reader = PyPDF2.PdfReader(file)
            
            # Search each page for the keyword
            for i in range(len(reader.pages)):
                ...
                if re.search(search_term, text, re.IGNORECASE):
                    # Add 1 to convert from 0-based to 1-based page numbering
                    matches.append(i + 1)
